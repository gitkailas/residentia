...
function DefaultersPage() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [units, cycles, payments] = await Promise.all([
        supabase.from("units").select("id, unit_no, floor, type, owner_name, billing_enabled").eq("status", "sold"),
        supabase.from("billing_cycles").select("id, unit_id, month, year, total_due, is_waiver_period"),
        supabase.from("payments").select("billing_cycle_id, total_paid"),
      ]);
  ...
  const rows = useMemo(() => {
    ...
    for (const p of data.payments) {
      ...
      paidByCycle.set(p.billing_cycle_id, (paidByCycle.get(p.billing_cycle_id) ?? 0) + Number(p.total_paid));
    }
    ...
    for (const c of data.cycles) {
      ...
      if (due <= 0) continue;
      const e = byUnit.get(c.unit_id) ?? { outstanding: 0, months: 0, oldest: `${c.month} ${c.year}` };
      e.outstanding += due;
      e.months += 1;
  ...
  function exportCsv() {
    ...
    rows.forEach((r: any) => {
      lines.push([r.unit_no, r.floor, r.type, (r.owner_name ?? "").replace(/,/g, " "), r.months, r.outstanding].join(","));
    });
    const blob = new Blob([lines.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `defaulters-${Date.now()}.csv`; a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="p-5 flex items-center gap-3">
          ...
          <div className="text-sm text-muted-foreground">Send reminders to listed units before next billing cycle.</div>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-muted-foreground">No defaulters. All units are up to date.</div>
        ) : (
              ...
              <tbody>
                {rows.map((r: any) => (
                  <tr key={r.id} className="border-t hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-status-unpaid">{inr(r.outstanding)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
