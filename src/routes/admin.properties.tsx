...
import { Plus, Pencil, Trash2, IndianRupee, Ruler, Search, Building2, Upload } from "lucide-react";
...
function PropertyDialog({
  ...
  async function handleBulkImport(rows: Record<string, unknown>[]) {
    ...
    const { error } = await db.from("units").upsert(withOwner, { onConflict: "unit_no" });
    setImportBusy(false);
    if (error) {
      toast.error(error.message);
      return;
    }
    toast.success(`Imported ${rows.length} apartment${rows.length !== 1 ? "s" : ""}`);
    qc.invalidateQueries({ queryKey: ["properties"] });
    qc.invalidateQueries({ queryKey: ["admin-dashboard"] });
    qc.invalidateQueries({ queryKey: ["apartment-names"] });
    onClose();
  ...
  return (
    <DialogContent className="max-w-2xl">
      <DialogHeader>
        <DialogTitle>
          {editing
            ? `Edit ${editing.property_name || editing.unit_no}`
            : "Add New Apartment"}
        </DialogTitle>
      </DialogHeader>
      ...
      {mode === "add" ? (
        ...
        <ImportExcelSection onImport={handleBulkImport} busy={importBusy} />
      )}
    </DialogContent>
  );
}
