              ...
              <tbody>
                {filtered.map((w: any) => (
                  <WaiverRow
                    key={w.id}
                    w={w}
                    onChange={() => qc.invalidateQueries({ queryKey: ["waivers"] })}
                  />
                ))}
              </tbody>
            </table>
  ...
  return (
    <tr className="border-t">
      <td className="px-4 py-3 font-semibold">
        {w.units?.unit_no}
        <div className="text-xs font-normal text-muted-foreground">{w.units?.owner_name}</div>
      </td>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={waiverType} onValueChange={setWaiverType}>
            ...
            <SelectContent>
              {["Manual", "Hardship", "Goodwill", "Discount", "Other"].map((t) => (
        ...
        <div className="rounded-lg border bg-muted/30 p-3 text-sm">
          Final payable: <strong>{inr(final)}</strong>
        </div>
        <div className="space-y-2">
          <Label>Reason</Label>
